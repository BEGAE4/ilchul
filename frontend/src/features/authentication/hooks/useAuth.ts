...
// 200: 로그인 상태 (userId/role/email), 401: 미로그인
  ...
  useEffect(() => {
    ...
    const checkLoginStatus = async () => {
      try {
        ...
        setAuthResult({ isLoggedIn: true, email: info.email, isAdmin, userId: info.userId ?? null });
      } catch {
        // 401 등 응답: 미로그인 상태로 처리
        if (cancelled) return;
        setAuthResult({ isLoggedIn: false, email: null, isAdmin: false, userId: null });
      }
    };
