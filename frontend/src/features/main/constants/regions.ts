// 홈·주변 목록에서 쓰는 시/도 16개와 대표 좌표.
// 광주와 전남은 한 항목이다 — 장소 주소가 둘 다 "전남광주통합특별시 …" 로 저장돼 서버가
// 시/도 단위로 가르지 못한다. `region=광주` 는 0건, `region=전남` 이 통합 주소 전체를
// 돌려준다(2026-09-21 운영 확인). 서버가 시군구로 나눠 주면 다시 두 항목으로 가른다.
//
// 주변 목록은 지역명(queryName ?? name)으로 조회한다 (buildNearbyQuery). 좌표는 GPS 위치가 어느 지역인지
// 판정할 때만 쓴다. 대표 좌표는 "행정 중심"이 아니라 **여행 수요가 몰리는 곳**으로 잡았다
// (예: 강원 → 춘천 대신 강릉권) — 사람들이 실제로 있을 법한 곳과의 거리로 판정하기 위해서다.
export interface Region {
  id: string;
  /** 화면에 보이는 짧은 이름 */
  name: string;
  /** 서버에 보내는 지역명. 화면 이름과 다를 때만 적는다 (없으면 name). */
  queryName?: string;
  /** 대표 좌표를 어디로 잡았는지 */
  landmark: string;
  /** 대표 좌표 — GPS 위치가 이 지역인지 판정할 때 쓴다. 목록 조회에는 쓰지 않는다. */
  lat: number;
  lng: number;
  /**
...
export const REGIONS: Region[] = [
  ...
  // 도청(무안) 대신 여행 수요가 큰 여수. 광주(상무지구)는 판정용 보조 좌표로 둔다.
  // id 는 'jeonnam' 그대로 — 전남을 골라 둔 사용자의 저장값이 유지된다.
  // 예전 'gwangju' 저장값은 findRegionById 가 null 을 돌려줘 위치 판정으로 넘어간다.
  {
    id: 'jeonnam', name: '광주·전남', queryName: '전남', landmark: '여수', lat: 34.7604, lng: 127.6622,
    detectAnchors: [[35.1521, 126.8514], [34.9506, 127.4872], [34.8118, 126.3922], [35.3211, 126.9882]],
  },
