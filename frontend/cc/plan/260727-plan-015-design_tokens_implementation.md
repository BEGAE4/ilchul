...
전 브랜치 최종 리뷰에서 흰 배경(`#ffffff`) 및 `--color-bg`(`#fafaf9`) 기준으로 주요 토큰의 대비비를 측정했다. 측정값(흰 배경 / `--color-bg` 기준):

| 토큰 | 흰 배경 대비 | `--color-bg` 대비 |
|---|---|---|
| `primary-400` | 2.68 | 2.57 |
| `primary-500` | 3.42 | 3.27 |
| `primary-600` | 4.73 | 4.52 |
| `primary-700` | 6.40 | 6.13 |
| `text-muted` | 4.83 | — |
| `text-strong` | 14.68 | — |
| `danger` | 3.76 | — |
| `success` | 2.28 | — |
...
**규칙 1 — `primary-500`은 브랜드·장식용으로만 사용한다.** 본문 크기 텍스트와 CTA 배경(흰 글씨)에는 **`primary-600` 이상**을 쓴다. `primary-500`은 3.42:1로 WCAG AA 본문 텍스트 기준(4.5:1)에 미달하며, 큰 텍스트·아이콘·테두리 등 3:1 기준에는 적합하다. 현재 `text-primary-500` 사용처 52곳은 2단계에서 일괄 정리한다.

**규칙 2 — `--color-success`(2.28:1)와 `--color-warning`(2.15:1)은 흰 글씨를 얹거나 흰 배경 위 텍스트 색으로 쓰지 않는다.** 어두운 글씨와 함께 배경(칩·뱃지)으로 쓰거나, 텍스트가 아닌 인디케이터(점, 아이콘, 테두리)로만 사용한다.
