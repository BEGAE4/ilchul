...
> Notion [엔드포인트 (3)](https://www.notion.so/3740e945fc0980c29284f38e962359db) 기준. 엔드포인트 17건.

| ID | 제목 | 메서드 | URL | 서버 | 클라이언트 | FE검토 |
| --- | --- | --- | --- | --- | --- | --- |
| 14 | 방문 인증 (스탬프) | POST | `/api/plan-place/{planPlaceId}/stamp` | 시작 전 | 시작 전 | N |
| 15 | 코스 신고 | POST | `/api/plan/{planId}/report` | 시작 전 | 시작 전 | N |
| 16 | 공유 링크 발급 | POST | `/api/plan/{planId}/share` | 시작 전 | 시작 전 | N |
| 29 | 플랜 장소 검색 | GET | `/api/place/search?keyword={keyword}` | 완료 | 시작 전 | N |
| 30 | 설문 바탕 플랜 장소 추천 | POST | `/api/place/recommend` | 완료 | 시작 전 | N |
| 39 | 플랜 후기 / 설명 작성 | POST | `` | 시작 전 | 시작 전 | N |
| 46 | 플랜 좋아요 | POST | `/api/like/{planId}` | 완료 | 시작 전 | Y |
| 47 | 플랜 상세 조회 | GET | `/api/plan/{planId}` | 완료 | 시작 전 | Y |
| 48 | 플랜 저장 전 시간계산 및 프리뷰 | POST | `/api/plan-place/duration` | 완료 | 시작 전 | Y |
| 49 | 플랜 스크랩 | POST | `/api/plan/scrapped/{planId}` | 완료 | 시작 전 | Y |
| 50 | 플랜 클론 (복사하기) | POST | `/api/plan/\{planId\}/clone` | 완료 | 시작 전 | N |
| 51 | 플랜 신규 생성 | POST | `/api/plan` | 시작 전 | 시작 전 | N |
| 52 | 플랜 수정 | PATCH | `/api/plan/{planId}` | 시작 전 | 시작 전 | N |
| 53 | 플랜 삭제 | DELETE | `/api/plan/{planId}` | 시작 전 | 시작 전 | N |
| 54 | 플랜 장소 순서 변경 | PATCH | `/api/plan-place/{planPlaceId}/order` | 시작 전 | 시작 전 | N |
| 55 | 플랜 장소 제거 | DELETE | `/api/plan-place/{planPlaceId}` | 시작 전 | 시작 전 | N |
| TBD | 플랜 장소 시간대/메모 수정 | PATCH | `/api/plan-place/{planPlaceId}` | 미정 | 미정 | N |
| TBD | 플랜 장소 교체 | PUT | `/api/plan-place/{planPlaceId}/replace` | 미정 | 미정 | N |
...
> **FE 제한 (ans-002)**: `phase === 'after'`(완료 코스)이면 `canEdit = false`로 수정 불가 처리. 단, `planTitle` 수정은 완료 후에도 허용해도 무방하므로 향후 `canEdit` 조건에서 분리 고려 중.
...
> **FE 제한 (ans-002)**: `phase === 'after'`이면 순서 편집 버튼 및 메뉴 모두 숨김 처리.
...
> **FE 제한 (ans-002)**: `phase === 'after'`이면 제거 버튼 숨김. 편집모드에서 다중 삭제 허용 (개별 제한의 자연스러운 예외).
...
> **FE 제한 (ans-002)**: `canVerify = phase === 'during'`일 때만 인증 버튼 노출. `phase === 'after'`이면 안내 문구(`'여행 기간이 종료되어 인증 및 수정이 불가합니다.'`) 표시.
...
# 플랜 장소 시간대/메모 수정 (TBD)
## URL : PATCH /api/plan-place/{planPlaceId}

> `MyCourseDetailPage` 장소 탭에서 개별 편집 모드 진입 시 호출 예정 (ans-002 UX 확정).

### 본문 파라미터

| **이름** | **유형** | **필수/선택** | **설명** |
| --- | --- | --- | --- |
| visitTime | String | 선택 | 방문 시각 (HH:mm) |
| stayDescription | String | 선택 | 장소 메모 (최대 200자) |

> **FE 제한 (ans-002)**: `phase === 'after'`이면 편집 불가.

---

# 플랜 장소 교체 (TBD)
## URL : PUT /api/plan-place/{planPlaceId}/replace

> `MyCourseDetailPage` 장소 탭에서 개별 장소 스왑 시 호출 예정 (ans-002 UX 확정).

### 본문 파라미터

| **이름** | **유형** | **필수/선택** | **설명** |
| --- | --- | --- | --- |
| newPlaceId | Integer | 필수 | 교체할 새 장소 ID |

> **FE 제한 (ans-002)**: `phase === 'after'`이면 교체 불가. URL 및 메서드는 백엔드 확정 전.
