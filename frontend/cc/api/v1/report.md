...
# REP-2. 내 신고 내역 조회 (Phase 3)
## URL : GET /api/reports/me

> **현재 UI 진입점은 없음.** 마이페이지 "내 신고 내역" 화면에서 사용 예정 (Phase 3, 별도 plan).
> 본인이 제출한 신고의 처리 상태를 추적하는 용도.

### 쿼리 파라미터

| **이름** | **유형** | **필수/선택** | **설명** |
| --- | --- | --- | --- |
| status | String | 선택 | `all` / `PENDING` / `REVIEWING` / `RESOLVED` / `REJECTED` (기본 `all`) |
...
| resolution | String | 선택. 처리 결과 (`BLINDED` / `WARNED` / `BANNED` / `NO_ACTION`). `status === 'RESOLVED'` 또는 `'REJECTED'` 일 때만 포함 |
...
> **현재 UI 진입점은 없음.** Phase 3 — 마이페이지 "내 신고 내역"(REP-2)에서 항목별 취소 액션으로 노출 예정.
> 본인이 제출한 신고만 취소 가능. 운영자가 이미 처리(RESOLVED/REJECTED)한 신고는 취소 불가.

**현재 UI 기준 결정 사항**:

- **PENDING / REVIEWING** 상태일 때만 취소 가능. **RESOLVED / REJECTED**는 `409`로 거절.
- 취소된 신고는 통계 노이즈 방지를 위해 **soft cancel**(`status: 'CANCELED'`)을 권장. 실삭제 여부는 백엔드 정책.
- 동일 사용자가 동일 대상에 재신고하려면 **취소가 선행되어야 함** (idempotency-key가 중복 차단)
...
| 403 | 본인 신고가 아님 |
| 404 | 신고를 찾을 수 없음 |
| 409 | 이미 처리 완료된 신고(RESOLVED/REJECTED) — 취소 불가 |
...
```java
{
  ...
  "data": {
    "reportId": "r-1024",
    "status": "CANCELED",
    "canceledAt": "2026-05-19T11:20:00Z"
  }
}
```
...
| **필드** | **유형** | **설명** |
| --- | --- | --- |
| reportId | String | 취소된 신고 식별자 |
| status | String | 취소 후 상태. `CANCELED` (백엔드 합의 필요 — REP-1의 `ReportStatus` enum에 추가 필요) |
